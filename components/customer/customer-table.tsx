...
const CustomerTable = forwardRef<CustomerTableRef, CustomerTableProps>(({
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm flex flex-col w-full" style={{ maxHeight: 'calc(100vh - 290px)' }}>
        ...
        <table className="w-full text-sm border-separate border-spacing-0 table-fixed">
          ...
          <thead className="sticky top-0 z-10">
            <tr className="text-left text-xs text-slate-500 font-semibold uppercase tracking-wider">
              <th className="px-4 py-3 w-24 text-center whitespace-nowrap bg-slate-50 border-b border-r border-slate-200">Mã HT</th>
              <th className="px-4 py-3 w-36 whitespace-nowrap bg-slate-50 border-b border-r border-slate-200">Mã Khách Hàng</th>
              <th className="px-4 py-3 w-[34%] whitespace-nowrap bg-slate-50 border-b border-r border-slate-200">Tên Khách Hàng (Tên Hiển Thị)</th>
              <th className="px-4 py-3 w-[24%] whitespace-nowrap bg-slate-50 border-b border-r border-slate-200">Tên Tiếng Anh</th>
              <th className="px-4 py-3 w-28 text-center whitespace-nowrap bg-slate-50 border-b border-r border-slate-200">TTKD</th>
              <th className="px-4 py-3 w-40 whitespace-nowrap bg-slate-50 border-b border-r border-slate-200">Người Phụ Trách</th>
              <th className="px-4 py-3 w-36 whitespace-nowrap bg-slate-50 border-b border-r border-slate-200">Ghi chú</th>
              <th className="px-4 py-3 w-20 text-center whitespace-nowrap bg-slate-50 border-b border-slate-200">Thao tác</th>
            </tr>
          </thead>
            ...
            {loading ? (
              ...
              filtered.map((customer, index) => (
                ...
                >
                  ...
                  {/* Tên KH */}
                  <td className="px-4 py-3.5">
                    <span className="font-semibold text-slate-800 truncate block" title={customer.name}>{customer.name}</span>
                  </td>

                  {/* Tên Tiếng Anh */}
                  <td className="px-4 py-3.5 text-xs text-slate-600">
                    {customer.ten_tieng_anh ? (
                      <span className="italic text-slate-700 truncate block" title={customer.ten_tieng_anh}>{customer.ten_tieng_anh}</span>
                    ) : (
      ...
      {/* Footer */}
      <div className="px-6 py-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500 bg-slate-50/50 shrink-0">
        <div>
